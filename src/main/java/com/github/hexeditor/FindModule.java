...
class FindModule extends Thread
{
	...
	public void run()
	{
		...
		if (this.v1 != null && this.v1.size() != 0)
		{
			...
			this.hexV.find2(this.pos,
					this.pos - (long) (this.isFound ? (this.ignoreCase ? this.realLength : this.inBytes.length) : 0));
		}
	}
		...
		} else
		{
			for (var3 = this.pile.size() - 1; -1 < var3; --var3)
			{
				...
				} else
				{
					...
					{
						int var5;
						for (var5 = 0; var5 < var6[2] && var5 < this.inChars[var6[0]][var4].length
								&& this.inChars[var6[0]][var4][var5] == this.inChars[var6[0]][var6[1]][var5]; ++var5)
						{
							;
						}
					...
					if (var6[0] + 1 == this.inChars.length && var6[1] + 1 == this.inChars[0].length
